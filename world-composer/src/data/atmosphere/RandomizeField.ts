...
export type RandomMethod = (grid: MACGrid, pos: Point) => Vector;
...
export const LeftWave: RandomMethod = (grid: MACGrid, [x, y]: Point) => [
    (x < grid.size / 2 ? DefaultRange : 0) + rand(),
    rand(),
];

export const RightWave: RandomMethod = (grid: MACGrid, [x, y]: Point) => [
    -(x > grid.size / 2 ? DefaultRange : 0) + rand(),
    rand(),
];

export const GlobalCurl: RandomMethod = (grid: MACGrid, [x, y]: Point) => [
